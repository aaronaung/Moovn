...
const useImageWithRetry = (
  ...
  const isLoading = imageStatus === IMAGE_STATUS.LOADING || imageStatus === IMAGE_STATUS.RETRYING;
  ...
  const isImageAlreadyLoaded = (image: HTMLImageElement) => {
    return (
      (image && image.complete && image.naturalWidth > 0) || new URL(image.src).protocol === "blob:"
    ); // skip blob images;
  };
